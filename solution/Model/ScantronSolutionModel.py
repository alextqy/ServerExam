...
class ScantronSolutionModel(BaseModel):
    EType: ScantronSolutionEntity = ScantronSolutionEntity
    ...
    def Update(self, _dbsession: DBsession, ID: int, Param: EType) -> Result:
        _result = Result()
        Data: ScantronSolutionEntity = _dbsession.query(self.EType).filter(self.EType.ID == ID).first()
        if Data is not None:
            try:
    ...
    def List(self, _dbsession: DBsession, Page: int, PageSize: str, ScantronID: int, Position: int) -> ResultList:
        _result = ResultList()
        _result.Status = True
        _result.Page = Page
